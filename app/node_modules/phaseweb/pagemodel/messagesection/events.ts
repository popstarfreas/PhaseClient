...
class MessageSectionEvents {
    ...
    private registerScroll(): void {
        $(ElementIdentifiers.ChatSection.scrollSection).on("scroll", (e) => {
            ...
            if (currentScroll <= 400) {
                ...
                } else if (this._client.getDiscussionSearch() !== null) {
    ...
    private handleScrolledTopSearch(): void {
        const search = this._client.getDiscussionSearch() as Search;
        const discussion = search.getDiscussion();
        if (this._messageSection.displayedMessagesCount >= 40 && !search.getInProgress()) {
            console.log(`Searching ${search.getTerms()} by ${search.getUsername()}@${search.getSystemName()}. Oldest: ${search.getOldestId()}`);
            this._client.requestMessageSearch(search.getTerms(), search.getOldestId() || undefined, search.getUsername(), search.getSystemName());
        }
    }
        ...
        searchForm.submit((e) => {
            ...
            if (rawSearchContent.length > 0) {
                ...
                if (usernameSeparator > -1) {
                    ...
                    systemName = this._client.getSystemName();
    ...
    private registerTitleClick(): void {
        ...
        title.on("click", () => {
            ...
            if (title.children().length === 0 && typeof this._client.getCurrentDiscussion() !== "undefined") {
     ...
     */
    private saveTitleChanges(): void {
        const title = $(ElementIdentifiers.ChatSection.title);
        if (title.children.length > 0) {
            const currentDiscussion = this._client.getCurrentDiscussion();
            if (typeof currentDiscussion !== "undefined") {
                const newTitle = title.children().first().val().trim();
                if (newTitle !== currentDiscussion.getName()) {
                    title.text(newTitle);
