...
class MessageSection {
    ...
    public updateDisplayedTitle(name: string): void {
        $(ElementIdentifiers.ChatSection.title).html(name);
    }
    ...
    private appendMessage(message: Message, scrollAfterImageLoad = true) {
            ...
            if (lastMessage === null || message.userId === System.joinLeaveMessageUserId) {
                ...
                if (this._lastMessageCombined !== null && this.messagesAreCombinable(message, this._lastMessageCombined)) {
                    ...
                    if (existingLiveTyping.length && this._client.configuration.mergeLiveTypingWithCompleteChatArea) {
                        addedMessage = $(Templates.ChatMessage(this._client, discussion, message));
                        existingLiveTyping.before(addedMessage);
                    } else {
                        addedMessage = $(Templates.ChatMessage(this._client, discussion, message));
                        $(ElementIdentifiers.ChatSection.list).append(addedMessage);
                    }
        ...
        if (addedMessage !== null && scrollAfterImageLoad) {
            ...
            const messageNoSpaces = message.content.asIs().replace(/ /g, "");
            if ((messageNoSpaces.match(/[\u0400-\u04FF]/g) || []).length > messageNoSpaces.length * 0.75 || message.getDisplayName().asIs() === "Glosario") {
                const untranslatedMessage = addedMessage.hasClass("chat-line") ? addedMessage : addedMessage.find(".chat-line").first();
                this._contextMenu.translator.translate(message.content.asIs(), "en")
                        .then((translatedMessage: string) => {
                            message.updateContent(translatedMessage);
                            untranslatedMessage.text(translatedMessage);
                        })
                        .catch((e: Error) => {
                            console.log(e);
                        });
    ...
    private appendToLastMessage(message: Message, last = true): JQuery {
        const element = $(`<div class="chat-line">${message.getContentStyled(this._client.currentDiscussion as Discussion)}</div>`);
        $(`${ElementIdentifiers.ChatSection.list} li:not('.in-progress'):last`)
    ...
    private prependToFirstMessage(message: Message, last = true): JQuery {
        return $(`${ElementIdentifiers.ChatSection.list} li:not('.in-progress'):first`)
            .find(".chat-message-content")
            .prepend(`<div class="chat-line">${message.getContentStyled(this._client.currentDiscussion as Discussion)}</div>`);
    }
    ...
    public displayMessage(message: Message): void {
        ...
        if (scrollIsAtBottom) {
            ...
            if (this._displayedMessages.length > 40 && (!this._client.configuration.shrinkJoinLeaveMessages || this._displayedMessages.length > 80)) {
                this.removeOldestMessages(this._displayedMessages.length - 40);
                if (this._client.currentDiscussion) {
                    this._client.currentDiscussion.viewingHistory = false;
                }
            }
            this.scrollToBottom();
        }
    ...
    public getScrollIsAtBottom(): boolean {
        const chatElement = $(ElementIdentifiers.ChatSection.scrollSection);
        const chatHtmlElement = chatElement[0] as HTMLElement;
        const maximumScroll = chatHtmlElement.scrollHeight - chatHtmlElement.offsetHeight;
        const currentScroll = chatHtmlElement.scrollTop;
    ...
    public scrollToBottom(): void {
        ...
        const chatHtmlElement = chatElement[0] as HTMLElement;
        const maximumScroll = chatHtmlElement.scrollHeight - chatHtmlElement.offsetHeight;
        chatHtmlElement.scrollTop = maximumScroll;
    }
    ...
    public updateOurLiveTyping(discussion: Discussion): void {
        ...
        const textInputElem = $(ElementIdentifiers.ChatInput.textInput)[0] as HTMLElement;
     ...
     */
    public removeLiveTyping(userId: string): void {
    ...
    public displayOlderMessages(): void {
        ...
        const messages = discussion.messages;
        const chatHtmlElement = chatElement[0] as HTMLElement;
        const oldMaximumScroll = chatHtmlElement.scrollHeight - chatHtmlElement.offsetHeight;
        ...
        if (messages.length > 40 && this._displayedMessages.length !== messages.length) {
            ...
            if (chatHtmlElement.scrollTop === 0) {
                const newMaximumScroll = chatHtmlElement.scrollHeight - chatHtmlElement.offsetHeight;
     ...
     */
    public removeLocalMessage(localMessageId: string): void {
    ...
    public displayDeepHistory(): void {
        ...
        this.scrollBy(offset.top - chatHtmlElement.clientHeight / 2);
    ...
    public saveTitleChanges(): void {
        ...
        if (title.children.length > 0) {
            const newTitle = (title.children().first().val() as string).trim();
            const currentDiscussion = this._client.currentDiscussion;
     ...
     */
    public focusChatInput(offset: number = 0): void {
        $(ElementIdentifiers.ChatInput.textInput).focus();
        ($(ElementIdentifiers.ChatInput.textInput)[0] as HTMLInputElement).setSelectionRange(offset, offset);
    }
}
