...
class PeopleSection {
    ...
    public refreshUsersOnlineList(): void {
        ...
        for (let i = 0; i < onlineUsers.length; i++) {
            user = onlineUsers[i];
            sectionList.append(new Templates.PeopleEntry(this._client.id, user, null).getContent());
        }
            ...
            for (const member of members) {
                if (member.status === UserStatus.Online) {
                    onlineUserElements.push({
                        element: new Templates.PeopleEntry(this._client.id, member as User, currentDiscussion),
    ...
    /**
     * Decides whether to remove the existing user add form
     *
     * @param username The username of the user who got added
     */
    public deciceRemoveUserAddForm(username: string, systemName: string): void {
        const form = $(ElementIdentifiers.PeopleSection.addDiscussionUserForm);
        if (form.length > 0) {
            const input = form.children("input").val();
            if (input.length > 0) {
                const formUsername = input.split("@")[0];
                const formSystemName = input.split("@")[1] || "";
                if (formUsername === username
                    && ((formSystemName.length === 0 && systemName === this._client.systemName)
                        || formSystemName === systemName)) {
                    form.remove();
                }
            }
        }
