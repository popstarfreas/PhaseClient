...
class MessageSection {
            ...
            if (message.getContent().asIs() === $(ElementIdentifiers.ChatInput.textInput).val()) {
                ...
                if (discussion) {
                    if (this._client.getConfiguration().mergeLiveTypingWithCompleteChatArea) {
                        ...
                        $(ElementIdentifiers.ChatSection.list).append(Templates.ChatMessage(this._client, discussion, message));
                    }
                }
                return;
            }
    ...
    public getScrollIsAtBottom(): boolean {
        ...
        const scrollIsAtBottom = currentScroll >= Math.ceil(maximumScroll);
    ...
    private displayOlderMessages(): void {
        ...
        const messages = discussion.getMessages();
        const displayedMessagesCount = $(ElementIdentifiers.ChatSection.list).children().length;
        ...
        if (messages.length > 40 && displayedMessagesCount !== messages.length) {
            const startIndex = messages.length - displayedMessagesCount - 1;
            ...
            this.scrollBy(newMaximumScroll - oldMaximumScroll);
        }
    }
