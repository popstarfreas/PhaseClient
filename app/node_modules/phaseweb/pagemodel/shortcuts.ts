...
class Shortcuts {
    ...
    constructor(client: Client) {
        this._combokeys = new Combokeys(document.documentElement as Element);
        GlobalBind(this._combokeys);
        this._client = client;
        ...
        this.bindShortcutsShow();
        this.bindJoinLeaveDisplay();
    }
    ...
    private preventShortcutInput(): void {
        (this._combokeys.stopCallback as any) = function(e, element, combo) {
            const chatInput = $(ElementIdentifiers.ChatInput.textInput);

            console.log(`${chatInput.length > 0} && ${chatInput.is(":focus")} && ${combo !== "esc"}`);

            return chatInput.length > 0 && chatInput.is(":focus") && combo !== "esc";
        };
    }

    private bindJoinLeaveDisplay(): void {
        let timeout: ReturnType<typeof setTimeout> | null = null;
        this._combokeys.bind("e p", () => {
            if (this._client.configuration.useAndDisplayJoinLeaveMessages) {
                $(".navbar-text").text("Disabled join/leave messages.");
                if (timeout != null) {
                    clearTimeout(timeout);
                }

                timeout = setTimeout(() => $(".navbar-text").text(""), 1000);
            } else {
                $(".navbar-text").text("Enabled join/leave messages.");
                if (timeout != null) {
                    clearTimeout(timeout);
                }
            ...
            this._client.configuration.useAndDisplayJoinLeaveMessages = !this._client.configuration.useAndDisplayJoinLeaveMessages;
        });
    }
    ...
    private bindShortcutsShow(): void {
        this._combokeys.bind("h", (e) => {
            $(ElementIdentifiers.Shortcuts.main).toggle();
        });

        $("body").on("click", (e) => {
            if ($(ElementIdentifiers.Shortcuts.main).has(e.target).length === 0
                && !$(ElementIdentifiers.Shortcuts.main).is(e.target)) {
                $(ElementIdentifiers.Shortcuts.main).hide();
            }
        });
    }
    ...
    private bindChatDefocus(): void {
        (this._combokeys as any).bindGlobal("esc", (e) => {
            const chatInput = $(ElementIdentifiers.ChatInput.textInput);
            chatInput.blur();
