...
function getUseSystemName(client: Client, message: Message): boolean {
    return message.systemName !== client.systemName && message.systemName !== null;
}
...
function getSystemNameTag(client: Client, message: Message): string {
    return getUseSystemName(client, message) ? `<span class="systemSuffix">@${message.systemName}</span>` : ``;
}
...
function getIpAddressElement(message: Message): string {
    console.log("Getting ip address " + message.ip);
...
function getUseAlert(client: Client, chatMessage: string): boolean {
    return chatMessage.indexOf(client.username) > -1;
}
