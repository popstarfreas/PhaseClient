...
class PeopleEntry {
    ...
    constructor(client: Client, user: User, discussion: Discussion | null) {
        ...
        if (discussion !== null) {
            this._isCreator = discussion.creator === user.id.toString();
    ...
    private isModerator(): boolean {
        if (this._discussion !== null) {
            return this._discussion.moderators[this._user.id];
        }
    ...
    private getRemoveUserIcon(): string {
        let icon = "";
        if (this._discussion !== null) {
            if (this.clientIsMod() && this._user.id.toString() !== this._client.id) {
            ...
            } else if (this._user.id.toString() === this._client.id) {
    ...
    public getContent(): string {
        const iconClass = this._user.status === UserStatus.Offline
                ? " user-status-offline"
                : this._user.status === UserStatus.Busy
                    ? " user-status-busy"
                    : "";
