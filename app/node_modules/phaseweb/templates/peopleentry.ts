...
class PeopleEntry {
    ...
    constructor(clientUserId: number, user: User, discussion: Discussion | null) {
        this._user = user;
        this._clientUserId = clientUserId;

        if (discussion !== null) {
            this._isCreator = discussion.creator === user.id;
            this._isModerator = discussion.moderators[user.id];
    ...
    private getModIcon(): string {
        ...
        if (this._discussion !== null) {
            if (this._isModerator) {
                if (this.clientIsMod()) {
                    icon = `<span class="mod demote-mod __web-inspector-hide-shortcut__" data-toggle="tooltip" data-placement="left" title="" data-original-title="Click to demote this user">♔</span>`;
                } else {
                ...
                icon = `<span class="add-mod" data-toggle="tooltip" data-placement="left" title="" data-original-title="Make user Moderator">♔</span>`;
            }
        }
    ...
    private getRemoveUserIcon(): string {
        let icon = "";
        if (this._discussion !== null) {
            if (this.clientIsMod() && this._user.id !== this._clientUserId) {
                icon = `<i class="removeuser-discussion fa fa-minus" data-toggle="tooltip" data-placement="left" title="" data-original-title="Remove User from Discussion"></i>`;
            } else if (this._user.id === this._clientUserId) {
                icon = `<i class="leaveuser-discussion fa fa-minus" data-toggle="tooltip" data-placement="left" title="" data-original-title="Leave Discussion"></i>`;
            }
        }
    ...
    private clientIsMod(): boolean {
        return this._discussion !== null &&
            (this._discussion.moderators[this._clientUserId] || this._discussion.creator === this._clientUserId);
    }
    ...
    public getContent(): string {
        ...
        const moderatorClass = this._isModerator ? ` ${ElementIdentifiers.PeopleSection.peopleUsernameMod.substr(1)}` : "";
