...
function DiscussionPreview(client: Client, discussion: Discussion, message: Message) {
    ...
    const chatMessage = (message.content as SafeString).withoutBbcodeAndHtml();
    ...
    const user = client.getPhaseUserByName(message.username.asIs());
    ...
    if (user) {
        avatarClass = `avatar_hash${Hash(user ? user.formatName : "")}`;
    } else {
        const prefix = message.systemName !== client.systemName ? `@${message.systemName}` : ``;
        avatarClass = `avatar_hash${Hash(`${message.username.asIs()}${prefix}`)}`;
    }
