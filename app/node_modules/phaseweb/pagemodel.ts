...
class PageModel {
    ...
    private loginLoadingTimeout: any;
    ...
    private messageInputHadCommandPrefix: boolean;
    private lastNotification: Notification | null;
    ...
    constructor(client: Client) {
        ...
        this.messageInputHadCommandPrefix = false;
    ...
    public showNotification(discussion: Discussion, text: string): void {
        ...
        Notification.requestPermission((result) => {
            if (result === 'granted') {
                ...
                let notification = new Notification(discussion.getName(), {
                ...
                notification.onclick = () => {
                    window.focus();
                    this.client.switchDiscussion(discussion.getId());
                    this.lastNotification = null;
                };
            }
        });
    ...
    public updateUsername(username: string): void {
        ...
        this.updateOurDisplayedName(username);
    }
    ...
    public updateDiscussionUsersList(resetScrollTop = false): void {
        ...
        if (currentDiscussion !== undefined) {
            ...
            for (let i = 0; i < members.length; i++) {
                let memberIsCreator = currentDiscussion.getCreator() === members[i].getId();
                let memberIsModerator = currentDiscussion.getModerators()[members[i].getId()];

                if (members[i].getStatus() === UserStatus.Online) {
                    onlineUserElements.push(Templates.PeopleEntry(<User>members[i], memberIsModerator, memberIsCreator));
                    onlineUsers++;
                } else {
                    offlineUserElements.push(Templates.PeopleEntry(<User>members[i], memberIsModerator, memberIsCreator));
                }
            }
    ...
    private appendMessage(lastMessage: Message | null, message: Message) {
        let discussion = this.client.getCurrentDiscussion();
        ...
        if (this.client.getConfiguration().combineMessages) {
            ...
            if (lastMessage == null || message.getUserId() == System.joinLeaveMessageUserId) {
                addedMessage = $(ElementIdentifiers.ChatSection.list).append(Templates.ChatMessage(this.client, discussion, message));
            } else {                
                ...
                if (lastMessage.getUserId() === message.getUserId()
                    ...
                    if (existingLiveTyping.length) {
                        addedMessage = existingLiveTyping.before(Templates.ChatMessage(this.client, discussion, message));
                    } else {
                        addedMessage = $(ElementIdentifiers.ChatSection.list).append(Templates.ChatMessage(this.client, discussion, message));
                    }
                }
        ...
        if (addedMessage !== null) {
            let _this = this;
            addedMessage.find('.autoLinkedImage').each(function() {
    ...
    public displayMessage(lastMessage: Message | null, message: Message): void {
        if (message.getUserId() === this.client.getId()) {
            if (message.getContent().asIs() === $(ElementIdentifiers.ChatInput.textInput).val()) {
                let discussion = this.client.getCurrentDiscussion();
                if (discussion) {
                    $(ElementIdentifiers.ChatInput.main).after(Templates.ChatMessage(this.client, discussion, message));
                }
                return;
            }
        }
    ...
    public displayLiveTyping(discussion: Discussion): void {
        ...
        for (let key in liveTypingMessages) {
            ...
            $(ElementIdentifiers.ChatSection.list).append(Templates.LiveTypingMessage(this.client, discussion, message));
        }
    ...
    public updateTerrariaUserStatus(name: string, online: boolean, tagColor?: string): void {
        if (online) {
            $(`.chat-terraria-${Hash(name, true)}`).removeClass(ElementIdentifiers.Message.terrariaOffline.substr(1)).addClass(ElementIdentifiers.Message.terrariaOnline.substr(1)).css('background', tagColor);
        } else {
            $(`.chat-terraria-${Hash(name, true)}`).removeClass(ElementIdentifiers.Message.terrariaOnline.substr(1)).addClass(ElementIdentifiers.Message.terrariaOffline.substr(1)).css('background', '');
        }
    }
    ...
    public updateOurChatColor(): void {
        ...
        if (currentDiscussion) {
            let isCreator = this.client.getId() === currentDiscussion.getCreator();
            let isMod = currentDiscussion.getModerators()[this.client.getId()];
     ...
     */
    private updateOurDisplayedName(name: string): void {
        $(ElementIdentifiers.ChatInput.username).text(name);
    }
    ...
    public switchSelectedDiscussion(discussionId: number): void {
        ...
        $(ElementIdentifiers.DiscussionSection.identifier(discussionId)).addClass('selected');
    }
    ...
    public updateDiscussionEntryMessagePreview(discussion: Discussion): void {
        ...
        $(ElementIdentifiers.DiscussionSection.identifier(discussion.getId()))
            ...
            .text(`${discussion.getRecentMessage().getDisplayName().asIs()}: ${messageContent.withBbcodeStripped()}`);
    }
    ...
    public showMessageLoading(): void {
        ...
        $(ElementIdentifiers.ChatSection.scrollSection).css('opacity', '0.3');
    }
    ...
    public hideMessageLoading(): void {
        ...
        $(ElementIdentifiers.ChatSection.scrollSection).css('opacity', '1');
    }
    ...
    private registerDiscussionListClick(): void {
        ...
        $(ElementIdentifiers.DiscussionSection.list).on('mousedown', ElementIdentifiers.DiscussionSection.discussion, function() {
            self.client.switchDiscussion(+$(this).attr('id').substr(4));
        });
    }
    ...
    private registerTextInput(): void {
        ...
        $(ElementIdentifiers.ChatInput.textInput).on('input', function(event) {
            ...
            if (currentMessage.indexOf('/') === 0) {
                if (!self.messageInputHadCommandPrefix) {
                    queue = TextDifference.getEditQueue(self.lastMessageInput, '');
                }
            } else {
                if (self.messageInputHadCommandPrefix) {
                    self.messageInputHadCommandPrefix = false;
