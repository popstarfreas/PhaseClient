...
class PageModel {
        ...
        if (this.client.getConfiguration().combineMessages) {
            ...
            if (lastMessage == null || message.getUserId() == System.joinLeaveMessageUserId) {
                ...
                if (lastMessage.getUserId() === message.getUserId()
                    ...
                    addedMessage = $(ElementIdentifiers.ChatSection.list).children('li').last().find('.chat-message-content').append("<br>"+message.getContentStyled());
                } else {
                    addedMessage = $(ElementIdentifiers.ChatSection.list).append(Templates.ChatMessage(this.client, discussion, message));
                }
            }
    ...
    public displayLiveTyping(discussion: Discussion): void {
        ...
        for (let key in liveTypingMessages) {
            ...
            $(ElementIdentifiers.ChatSection.liveTyping).append(Templates.LiveTypingMessage(this.client, discussion, message));
        }
    ...
    public updateTerrariaUserStatus(name: string, online: boolean): void {
        if (online) {
            $(`.chat-terraria-${Hash(name, true)}`).removeClass(ElementIdentifiers.Message.terrariaOffline.substr(1)).addClass(ElementIdentifiers.Message.terrariaOnline.substr(1));
        } else {
            $(`.chat-terraria-${Hash(name, true)}`).removeClass(ElementIdentifiers.Message.terrariaOnline.substr(1)).addClass(ElementIdentifiers.Message.terrariaOffline.substr(1));
        }
    }
