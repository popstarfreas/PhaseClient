...
import TerrariaUsersOnline from 'phase/messagetypes/terrariausersonline';
import TerrariaJoinLeave from 'phase/messagetypes/terrariajoinleave';
import TerrariaChangeName from 'phase/messagetypes/terrariachangename';
...
class Network {
    ...
    private switchDiscussionTo: number | null;
    ...
    private handleReconnect(): void {
        ...
        if (this.client.getSession().length > 0) {
            ...
            if (typeof currentDiscussion !== 'undefined') {
                this.switchDiscussionTo = currentDiscussion.getId();
            }
    ...
    private handleDiscussionsList(discussions: Discussions): void {
        ...
        if (this.switchDiscussionTo !== null) {
            this.client.switchDiscussion(this.switchDiscussionTo);
            this.switchDiscussionTo = null;
        }
    }
    ...
    private handleChatEditQueue(editQueue: EditQueue): void {
        ...
        if (typeof storedLiveTyping === 'undefined') {
            if (messageContent.trim().length > 0) {
                ...
                this.client.handleNewLiveTyping(discussion, discussion.getLiveTypingMessages()[userId]);
            }
        } else {
            if (messageContent.trim().length > 0) {
                discussion.updateLiveTypingMessageContent(userId, messageContent);
                this.client.handleUpdateLiveTyping(discussion, discussion.getLiveTypingMessages()[userId]);
            } else {
    ...
    private handleTerrariaJoin(joinInfo: TerrariaJoinLeave): void {
        this.client.handleTerrariaJoin(joinInfo.username);
    }
