...
class Network {
    ...
    constructor(client: Client, options: object) {
        ...
        this.socket.on("discussions list", (discussions) => {
            if (this.isGuest) {
                this.systemChannel = this.socket.subscribe(`System:${this.client.getSystemName()}`);
            }
    ...
    public sendGuestRequest(systemName: string): void {
        this.isGuest = true;
        this.client.setSystemName(systemName);
    ...
    private handleDiscussionMessages(discussionMessages: DiscussionMessages): void {
        ...
        for (const message of messages) {
            ...
            if (this.client.getConfiguration().useAndDisplayJoinLeaveMessages
                ...
                || message.tag === null) {
                messageList.push(new Message(message.ID,
                        message.userID,
                        message.avatar,
                        username, guestname,
                        message.content,
            ...
            const oldestIdReceived = discussion.getOldestMessageIdReceived();
            if (oldestIdReceived === -1 || message.ID < oldestIdReceived) {
                discussion.setOldestMessageIdReceived(message.ID);
            }
            user = null;
        }
    ...
    private handleDiscussionUserAdd(discussionUserAdd: DiscussionUserAdd): void {
        this.client.handleDiscussionUserAdd(discussionUserAdd);
    }
    ...
    private handleDiscussionUserRemove(discussionUserRemove: DiscussionUserRemove): void {
        this.client.handleDiscussionUserRemove(discussionUserRemove);
    }
    ...
    private handleSearchResults(searchResults: SearchResults): void {
        this.client.handleSearchResults(searchResults);
    }
     ...
     */
    public requestSearchResults(discussionId: number, terms: string, username?: string, systemName?: string): void {
        this.socket.emit("search discussion messages", {
            discID: discussionId,
            terms,
            includeJoinLeaveMessages: this.client.getConfiguration().useAndDisplayJoinLeaveMessages,
            username,
            systemName
        });
    }
