...
class Network {
    ...
    private handleChatEditQueue(editQueue: EditQueue): void {
        ...
        if (typeof storedLiveTyping === 'undefined') {
            if (messageContent.trim().length > 0) {
                ...
                this.client.handleNewLiveTyping(discussion, <Message>discussion.getLiveTypingMessages()[userId]);
            }
        } else {
            if (messageContent.trim().length > 0) {
                discussion.updateLiveTypingMessageContent(userId, messageContent);
                this.client.handleUpdateLiveTyping(discussion, <Message>discussion.getLiveTypingMessages()[userId]);
            } else {
    ...
    private handleNewDiscussion(newDiscussion: NewDiscussion): void {
        let recentMessage = new Message(0, "", "", "", newDiscussion.lastMessage, 0, "", "", "");
