...
function autoLinkFormat(match, link, protocol, www, domain, regexDotGroup, tld, port, filepath, fileExtension, getParams, endChar): string {
        ...
        if (videoSites.indexOf(domain) > -1 && (typeof getParams !== "undefined" && getParams.length > 0 || domain === "youtu" && tld === ".be")) {
            ...
            output = parseMediaLink(output, link, endChar);
        } else if (domain === "gyazo" && typeof filepath !== "undefined" && filepath.length === 33) {
            output = `<a href="https://gyazo.com${filepath}">https://gyazo.com${filepath}</a><br><img class="autoLinkedImage" src="https://gyazo.com${filepath}.png"/>${endChar}`;
        } else if (domain === "puu" && tld === ".sh" && typeof filepath !== "undefined" && filepath.length === 17) {
            output = `<a href="https://puu.sh${filepath}">https://gyazo.com${filepath}</a><br><img class="autoLinkedImage" src="https://puu.sh${filepath}.png"/>${endChar}`;
        } else {
...
function parseMediaLink(output: string, link: string, endChar: string) {
    ...
    output = link.replace(imageExpression, function(url) {
        if (url.indexOf("https") === -1) {
            ...
            return `<img class="autoLinkedImage" src="${url}"/>${endChar}`;
        }
    });
