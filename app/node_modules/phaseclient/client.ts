...
class Client {
    ...
    public switchDiscussionTo: string | null;
    ...
    public getNewMessageId(): string {
        return (this._uniqueMessageId++).toString();
    }
    ...
    public getPhaseUser(userId: string): User | undefined {
        return this.phaseUsers[userId];
    }
    ...
    public addPhaseUser(user: User): void {
        this.phaseUsers[user.id.toString()] = user;
    }
    ...
    public switchDiscussion(discussionId: string): void {
        ...
        if (this._currentDiscussion) {
            ...
            this._currentDiscussion.viewingHistory = false;
        }
    ...
    public handleChatMessage(discussion: Discussion, message: Message): void {
        ...
        discussion.removeLiveTypingMessage(parseInt(message.userId));
    ...
    public handleUsersOnline(userList: User[]): void {
        ...
        for (const user of userList) {
            if (!phaseUsers[user.id.toString()]) {
    ...
    public handleDiscussionList(receivedDiscussions: Discussions): void {
        ...
        for (let i = receivedDiscussions.length - 1; i >= 0; i--) {
            ...
            message = new Message(discussionInfo.MessageID ? discussionInfo.MessageID.toString() : this.getNewMessageId(),
                discussionInfo.UserID ? discussionInfo.UserID.toString() : "0",
                "",
                ...
                "",
                "");
            discussion = new Discussion(discussionInfo.ID.toString(), discussionInfo.Name, discussionInfo.Creator.toString(), [], message);
        ...
        if (this.switchDiscussionTo !== null) {
            ...
            discussions.reverse().forEach((d, i) => {
                if (d !== lastDiscussion && i <= 10) {
                    this._network.requestDiscussionDetails(d.id, this.configuration.useAndDisplayJoinLeaveMessages);
                }
            });
        }
    ...
    public handleTerrariaUsers(users: TerrariaUsersOnline): void {
        for (const user of users) {
            this._terrariaUsers[user.name] = new TerrariaUser(user.name, user.tagName, user.tagColour);
        }
    }
    ...
    public handleTerrariaJoin(info: TerrariaJoinLeave): void {
        this._terrariaUsers[info.username] = new TerrariaUser(info.username, info.tagName, info.tagColour);
    }
    ...
    public handleTerrariaLeave(info: TerrariaJoinLeave): void {
        delete this._terrariaUsers[info.username];

        const user = this.getPhaseUserByName(info.username);
        if (user !== null) {
    ...
    public handleTerrariaChangeName(oldUsername: string, newUsername: string): void {
        if (this._terrariaUsers[oldUsername]) {
            const user = this._terrariaUsers[oldUsername];
            const info = { username: user.displayName, tagName: user.tagName, tagColour: user.tagColour};
            this.handleTerrariaLeave(info);
            this.handleTerrariaJoin({ ...info, username: newUsername });
        }
    }
    ...
    public handleDiscussionUserAdd(discussionUserAdd: DiscussionUserAdd): void {
        ...
        let user = this.getPhaseUser(discussionUserAdd.userID.toString());
        if (typeof discussion !== "undefined") {
            if (typeof user === "undefined") {
                user = new User(discussionUserAdd.userID, discussionUserAdd.username,
                    ...
                    this.systemName, UserStatus.Offline, this.configuration);
                this.addPhaseUser(user);
            }
    ...
    public handleDiscussionUserPromote(discussionUserPromote: DiscussionUserPromote): void {
        const discussion = this.discussions[discussionUserPromote.discID.toString()];
        const user = this.getPhaseUserByName(discussionUserPromote.username);
        if (typeof discussion !== "undefined") {
            if (user !== null) {
                discussion.moderators[user.id.toString()] = true;
            }
        }
    }
    ...
    public handleDiscussionUserDemote(discussionUserDemote: DiscussionUserDemote): void {
        const discussion = this.discussions[discussionUserDemote.discID.toString()];
        const user = this.getPhaseUserByName(discussionUserDemote.username);
        if (typeof discussion !== "undefined") {
            if (user !== null) {
                delete discussion.moderators[user.id.toString()];
            }
        }
    }
    ...
    public requestMessageSearch(terms: string, oldestId?: string, username?: string, systemName?: string): boolean {
        ...
        const deepHistoryDiscussion = this.deepHistory !== null  && this.deepHistory.discussion !== null ? this.deepHistory.discussion : undefined;
        const currentDiscussion = this.currentDiscussion || searchDiscussion || deepHistoryDiscussion;
        if (search !== null && search.oldestId === null && terms === search.terms) {
            return false;
        }
        ...
        if (typeof currentDiscussion !== "undefined") {
            ...
            this._deepHistory = null;
            this._search.retrievalInProgress = true;
    ...
    public handleSearchResults(searchResults: SearchResults): void {
        ...
        if (search !== null) {
            if (searchResults.results.length !== 0) {
                search.oldestId = searchResults.smallestResultID.toString();
                const messages: Message[] = [];
                for (const searchResult of searchResults.results) {
                    const message = new Message(
                        searchResult.messageID.toString(),
                        searchResult.userID.toString(),
                        searchResult.avatar,
                        searchResult.guest ? "" : searchResult.username,
                        searchResult.guest ? searchResult.username : "",
                        searchResult.content,
                        searchResult.timestamp,
                        searchResult.systemName,
                        searchResult.tag || "",
                        searchResult.tagcolour || ""
                    );

                    if (searchResult.IP !== null) {
                        message.ip = searchResult.IP;
                    }
            ...
            search.retrievalInProgress = false;
        }
    }
    ...
    /**
     * Requests the deep history of a message from a discussion
     */
    public requestDiscussionDeepHistory(discussion: Discussion, message: Message): void {
        this._deepHistory = new DeepHistory(discussion, message.id);
        this._deepHistory.retrievalInProgress = true;
        this._search = null;
        this._currentDiscussion = undefined;
        this._network.requestSurroundingHistory(message.id);
    }
    ...
    public requestMessageDeepHistory(messageId: string): void {
        this._deepHistory = new DeepHistory(null, messageId);
        this._deepHistory.retrievalInProgress = true;
        this._search = null;
        this._currentDiscussion = undefined;
        this._network.requestSurroundingHistory(messageId);
    }
    ...
    public requestDiscussionDeepHistoryOlder(): void {
        ...
        if (deepHistory !== null && deepHistory.discussion !== null) {
            deepHistory.retrievalInProgress = true;
            this._network.requestDeepHistoryOlder(deepHistory.discussion.id, deepHistory.oldestId);
        }
    }
    ...
    public requestDiscussionDeepHistoryNewer(): void {
        ...
        if (deepHistory !== null && deepHistory.discussion !== null) {
            deepHistory.retrievalInProgress = true;
            this._network.requestDeepHistoryNewer(deepHistory.discussion.id, deepHistory.newestId);
        }
    }
    ...
    public handleDeepHistory(deepHistoryResults: DeepHistoryResults): void {
        ...
        const messages: Message[] = [];
        const deepHistoryMessages = deepHistoryResults.messages.reverse();
        for (const deepHistoryMessage of deepHistoryMessages) {
            const message = new Message(
                deepHistoryMessage.messageID.toString(),
                deepHistoryMessage.userID.toString(),
                deepHistoryMessage.avatar,
                deepHistoryMessage.guest ? "" : deepHistoryMessage.username,
                deepHistoryMessage.guest ? deepHistoryMessage.username : "",
                deepHistoryMessage.content,
                deepHistoryMessage.timestamp,
                deepHistoryMessage.systemName,
                deepHistoryMessage.tag || "",
                deepHistoryMessage.tagcolour || ""
            );

            if (deepHistoryMessage.IP !== null) {
                message.ip = deepHistoryMessage.IP;
            }
            messages.push(message);
        }

        this.deepHistory.addMessages(messages);
        this.deepHistory.retrievalInProgress = false;
        this.deepHistory.oldestId = deepHistoryResults.oldestID.toString();
        this.deepHistory.newestId = deepHistoryResults.newestID.toString();
        this.deepHistory.discussion = this.discussions[deepHistoryResults.discID.toString()];
    }
    ...
    public handleDeepHistoryOlder(deepHistoryOlderResults: DeepHistoryOlderResults): void {
        ...
        if (deepHistoryOlderResults.messages.length === 0) {
            this.deepHistory.obtainedOldest = true;
            return;
        }

        const messages: Message[] = [];
        for (const deepHistoryMessage of deepHistoryOlderResults.messages) {
            const message = new Message(
                deepHistoryMessage.messageID.toString(),
                deepHistoryMessage.userID.toString(),
                deepHistoryMessage.avatar,
        ...
        this.deepHistory.prependMessages(messages);
        this.deepHistory.retrievalInProgress = false;
        this.deepHistory.oldestId = deepHistoryOlderResults.messages[deepHistoryOlderResults.messages.length - 1].messageID.toString();
    }
    ...
    public handleDeepHistoryNewer(deepHistoryNewerResults: DeepHistoryNewerResults): void {
        ...
        const messages: Message[] = [];
        for (const deepHistoryMessage of deepHistoryNewerResults.messages) {
            const message = new Message(
                deepHistoryMessage.messageID.toString(),
                deepHistoryMessage.userID.toString(),
                deepHistoryMessage.avatar,
        ...
        this.deepHistory.addMessages(messages);
        this.deepHistory.retrievalInProgress = false;
        this.deepHistory.newestId = deepHistoryNewerResults.messages[deepHistoryNewerResults.messages.length - 1].messageID.toString();
    }
    ...
    public createDiscussion(discussionName: string): void {
        this._network.requestCreateDiscussion(discussionName);
    }

    /**
     * Adds a user to the current discussion
     *
     * @param username The username of the user to add
     * @param systemName The system name of the user to add
     */
    public addDiscussionUser(username: string, systemName: string): void {
        if (typeof this.currentDiscussion !== "undefined") {
            if (systemName.length === 0) {
                systemName = this.systemName;
            }
            this._network.requestAddDiscussionUser(this.currentDiscussion.id, username, systemName);
        }
    }

    /**
     * Leave the current discussion
     */
    public leaveCurrentDiscussion(): void {
        if (typeof this.currentDiscussion !== "undefined") {
            this._network.requestLeaveDiscussion(this.currentDiscussion.id);
        }
    }

    /**
     * Removes a user from the current discussion
     *
     * @param username The username of the user to remove
     * @param systemName The system name of the user to remove
     */
    public removeUserFromCurrentDiscussion(username: string, systemName: string): void {
        if (typeof this.currentDiscussion !== "undefined" && username.length > 0) {
            this._network.requestRemoveDiscussionUser(this.currentDiscussion.id, username, systemName);
        }
    }

    /**
     * Promote a user in the current discussion
     *
     * @param username The username of the user to promote
     * @param systemName The system name of the user to promote
     */
    public promoteUserInCurrentDiscussion(username: string, systemName: string): void {
        if (typeof this.currentDiscussion !== "undefined") {
            this._network.requestPromoteDiscussionUser(this.currentDiscussion.id, username, systemName);
        }
    }

    /**
     * Demotes a user in the current discussion
     *
     * @param username The username of the user to demote
     * @param systemName The system name of the user to demote
     */
    public demoteUserInCurrentDiscussion(username: string, systemName: string): void {
        if (typeof this.currentDiscussion !== "undefined") {
            this._network.requestDemoteDiscussionUser(this.currentDiscussion.id, username, systemName);
        }
    }

    /**
     * Request a user search using the given terms
     *
     * @param terms The search terms
     */
    public requestSearchUser(terms: string): void {
        this._network.requestUsernameAutocomplete(terms);
    }
    ...
    public handleUserAvatarUpdate(userAvatarUpdate: UserAvatarUpdate): void {
        const user = this._phaseUsers[userAvatarUpdate.userID.toString()];
        if (typeof user !== "undefined") {
            if (user.id.toString() === this.id) {
                this.avatar = userAvatarUpdate.avatar;
            }

            user.avatar = userAvatarUpdate.avatar;
        }
    }
