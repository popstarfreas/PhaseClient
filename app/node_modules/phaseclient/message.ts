...
class Message extends CoreMessage {
    ...
    public getFormattedTimestamp(): string {
        if (!this.formattedTimestamp)  {
            this.formattedTimestamp = Moment(this.getTimestamp()*1000).calendar("", {
                sameDay: "[Today at] LT",
                lastWeek: "dddd [at] LT",
    ...
    public getContentStyled(): string {
        if (!this.contentStyled) {
            this.contentStyled = BBCode.convertText(this.getContent().getContentWithHtmlEscaped());
        }

        return this.contentStyled;
    }
