...
class Message extends CoreMessage {
    ...
    private _localMessageId: string;
    protected _isInProgress: boolean;

    constructor(id: string, userId: string, avatar: string, username: string, guestname: string,
                content: string, timestamp: number, systemName: string, tag: string, tagColour: string) {
        super(id, userId, avatar, username, guestname, content, timestamp, systemName, tag, tagColour);

        this._username = new SafeString(username);
        this._guestname = new SafeString(guestname);
        this._content = new SafeString(content);
        this._isInProgress = false;
        this._localMessageId = (Message.curlocalMessageId++).toString();
    }

    public get localMessageId(): string {
