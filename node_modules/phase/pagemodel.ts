...
class PageModel {
    ...
    public displayMessages(discussion: Discussion): void {
        ...
        for (let i = messages.length-1; i >= 0 && messagesDisplayed < 40; i--) {
            ...
            if (this.client.getConfiguration().combineMessages) {
                ...
                if (i == messages.length-1 || messages[i].getUserId() == System.joinLeaveMessageUserId) {
                    $(ElementIdentifiers.ChatSection.list).prepend(Templates.ChatMessage(this.client, messages[i]));
                } else {                
                    ...
                    if (messages[i+1].getUserId() === messages[i].getUserId()
                        ...
                        $(ElementIdentifiers.ChatSection.list).children('li').first().find('.chat-message-content').prepend(messages[i].getContentStyled()+"<br>");
                    } else {
                        $(ElementIdentifiers.ChatSection.list).prepend(Templates.ChatMessage(this.client, messages[i]));
                    }
                }
