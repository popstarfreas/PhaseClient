...
class Network {
    ...
    private handleChatMessage(message: ChatMessage): void {
        ...
        if (this.client.getPhaseUsers()[message.userID]) {
            ...
            if (!message.guest) {
                user = new User(message.userID, message.username, message.systemName, message.avatar, this.client.getSystemName());
                this.client.addPhaseUser(user);
            }
        }
        ...
        discussion.appendMessage(chatMessage);

        if (discussion.getIsBeingViewed()) {
            this.client.getPageModel().appendMessage(chatMessage);
        } else {
            this.client.getPageModel().setDiscussionUnread(discussion);

            // Doesn't work for mobile
            if (!this.client.getPageModel().getWindowHasFocus() && chatMessage.getContent().withBbcodeStripped().indexOf(this.client.getUsername()) > -1) {
                this.client.getPageModel().showNotification(chatMessage.getContent().withBbcodeStripped());
            }
        }

        discussion.setRecentMessage(chatMessage);
        this.client.getPageModel().updateDiscussionEntryMessagePreview(discussion);
    }
    ...
    private handleChatEditQueue(editQueue: EditQueue): void {
        ...
        let messageContent = "";
        let chatUser: User = this.client.getPhaseUsers()[userId];
        let storedLiveTyping = discussion.getLiveTyping()[userId];
        ...
        if (typeof storedLiveTyping === 'undefined') {
            if (messageContent.trim().length > 0) {
                discussion.setLiveTyping(userId, new Message(userId, chatUser.getAvatar(), chatUser.getName(), "", messageContent, Date.now()/1000, systemName, "", ""));
                
                if (discussion.isBeingViewed) {
                    this.client.getPageModel().createLiveTyping(discussion.getLiveTyping()[userId]);
                }
            }
        } else {
            if (messageContent.trim().length > 0) {
                discussion.updateLiveTypingContent(userId, messageContent);
                
                if (discussion.isBeingViewed) {
                    this.client.getPageModel().updateLiveTyping(discussion.getLiveTyping()[userId]);
                }
            } else {
                discussion.removeLiveTyping(userId);
                
                if (discussion.isBeingViewed) {
                    this.client.getPageModel().removeLiveTyping(userId);
                }
            }
        }
    }
