...
class Network {
    ...
    private handleChatMessage(message: ChatMessage): void {
        ...
        if (discussion.getIsBeingViewed()) {
            ...
            this.client.getPageModel().appendMessage(lastMessage, chatMessage);
            this.client.getPageModel().removeLiveTyping(message.userID);
        } else {
    ...
    private handleChatEditQueue(editQueue: EditQueue): void {
        ...
        if (typeof storedLiveTyping === 'undefined') {
            if (messageContent.trim().length > 0) {
                ...
                if (discussion.getIsBeingViewed()) {
                    this.client.getPageModel().createLiveTyping(discussion.getLiveTypingMessages()[userId]);
                }
            }
        } else {
            if (messageContent.trim().length > 0) {
                ...
                if (discussion.getIsBeingViewed()) {
                    this.client.getPageModel().updateLiveTyping(discussion.getLiveTypingMessages()[userId]);
                }
            } else {
    ...
    private handleUsersOnline(usersOnline: UsersOnline): void {
        ...
        for (let i = 0; i < usersOnline.length; i++) {
            ...
            if (!phaseUsers[user.UserID]) {
                this.client.addPhaseUser(new User(user.UserID, user.name, user.systemName, user.Avatar, this.client.getSystemName()));
            }
        }
    }
